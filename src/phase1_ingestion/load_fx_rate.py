# load_fx_rate.py
# โหลด FX Rate CSV เข้า PostgreSQL schema: raw
...
def verify(engine, schema: str, table: str, expected: int):
    ...
    if actual == expected:
        ...
        raise ValueError(f"Reconciliation ล้มเหลว: DB={actual:,} ≠ CSV={expected:,}")

# Main
def main():
    SOURCE_FILE = "data/raw/FX_Rate_Monthly_2024.csv"
    SCHEMA      = "raw"
    TABLE       = "fx_rate"

    log.info("=" * 50)
    log.info("เริ่ม FX Rate Ingestion Pipeline")
